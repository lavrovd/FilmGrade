    ...
    float p_Display, const float* p_Input, float* p_Output)
{
   ...
   if ((x < p_Width) && (y < p_Height))
   {
	   ...
	   float outR = p_Display != 1.0f ? satR : y / height >= p_ShadP && y / height <= p_ShadP + 0.005f ? (fmodf(x, 2.0f) != 0.0f ? 1.0f : 0.0f) : satR >= (y - 5) / height && satR <= (y + 5) / height ? 1.0f : 0.0f;
	   float outG = p_Display != 1.0f ? satG : y / height >= p_HighP && y / height <= p_HighP + 0.005f ? (fmodf(x, 2.0f) != 0.0f ? 1.0f : 0.0f) : satG >= (y - 5) / height && satG <= (y + 5) / height ? 1.0f : 0.0f;
	   float outB = p_Display != 1.0f ? satB : y / height >= p_ContP && y / height <= p_ContP + 0.005f ? (fmodf(x, 2.0f) != 0.0f ? 1.0f : 0.0f) : satB >= (y - 5) / height && satB <= (y + 5) / height ? 1.0f : 0.0f;
